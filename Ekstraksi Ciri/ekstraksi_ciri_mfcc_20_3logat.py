...
for file in range(0, len(audio_files), 1):
    ...
    #Proses MFCC
    MFCC= mfcc(cA2, sfreq, winlen=0.02, winstep=0.01, numcep=20, winfunc=np.hamming, nfft=1024)
    ...
    final_1 = np.reshape(pca_result[:,:2], 40, order='F')
    final =  final_1
    
    #Export Ke CSV
    f =  open('feature_mfcc20_all_data.csv', 'a')
    with f :
        w = csv.writer(f)
        w.writerow(final)